...
function ReportIrregularPayment() {
    ...
    $.post(directories.reportProduct.GetReportProductIrregularPayment, param)
        .done(function (data) {
            if (data.status !== "error") {
                ...
                $.each(data, function (key, value) {
                    ...
                    _html += '<tr><td>' + value.name + '</td><td>' + value.zone + '</td><td >' + value.idPrestamo + '</td><td >' + value.number + '</td><td >' + value.amount + '</td><td>' + value.PaymentObservation + '</td><td>' + value.observationPartial + '</td>';
